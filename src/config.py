...
def load_config():
    ...
    assert (
        "client_id" in data and "api_key" in data and "username" in data
    ), "config.json is missing required fields: client_id, api_key, username"
    ...
    assert (
        client_id != "" and client_id != "your_client_id"
    ), "Please enter your client_id in the config.json file"
    assert (
        api_key != "" and api_key != "your_api_key"
    ), "Please enter your client_id in the config.json file"
    assert (
        username != "" and username != "your_username#1234"
    ), "Please enter your username in the config.json file"
